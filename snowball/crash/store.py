"""Isolated Crash Guard ledger — short lots only (side=short)."""
...
def _row_strategy(row: sqlite3.Row) -> str:
    try:
        ...
        return "crash_guard"
    return str(val) if val else "crash_guard"


class CrashStore:
    """Sqlite book for Crash Guard shorts. Leverage-1 margin accounting."""
    ...
    def _init_schema(self) -> None:
        self._conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS positions (
                ...
                exit_price REAL,
                realized_pnl REAL,
                strategy TEXT NOT NULL DEFAULT 'crash_guard'
            );
    ...
    def equity_usd(self, marks: dict[str, float]) -> float:
        """Cash + margin + short MTM: cash + notional + (entry - mark)*qty."""
        cash = self.cash_usd()
        total = cash
        for pos in self.open_positions():
            px = marks.get(pos.product, pos.entry_price)
            # Locked margin (notional) + unrealized short PnL
    ...
    def open_short(
        ...
        strategy: str = "crash_guard",
    ) -> tuple[Position, Fill]:
        """Open a short: lock notional as margin (lev=1), record sell fill."""
    ...
    def cover_short(
        ...
        """Cover short: return margin + short PnL (entry - exit)*qty - fee."""
        ts = ts or utcnow()
        with self._lock:
            row = self._conn.execute(
            ...
            # Return locked margin + unrealized that is now realized
    ...
    def scorecard(self) -> dict:
        ...
        by_strategy = _bucket(lambda p: p.strategy or "crash_guard")
